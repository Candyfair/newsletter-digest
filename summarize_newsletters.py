...
SUMMARY_PROMPT_EN = """You are an assistant that summarizes newsletters.

Read the newsletter content below and write a summary following these rules:
- Output language: ENGLISH only. Write as a native English speaker.
...
def assign_themes(newsletters: list[Newsletter]) -> None:
    """
    Assign a theme to each newsletter based on keyword scoring.

    Instead of stopping at the first matching theme, every theme is scored
    by counting how many of its keywords appear in the haystack.
    The theme with the highest score wins. Ties are broken by insertion order
    (i.e. the theme declared first in THEME_RULES takes priority).

    Search order in the haystack: sender → subject → summary.
    """
    THEME_RULES: dict[str, list[str]] = {
        "Intelligence artificielle": [
            ...
            "agi", "intelligence artificielle", "modèle de langage", "génératif",
        ...
        "Développement web & logiciel": [
            ...
            "nextjs", "typescript", "node.js", "python", "software", "developer",
            "engineering", "github", "api", "backend",
            "développeur", "logiciel", "programmation",
        ],
        "Productivité & outils": [
            "productivity", "notion", "obsidian", "pkm", "tools",
            "workflow", "automation", "habit", "focus",
            "productivité", "outils", "méthode",
        ],
        "Startups & tech business": [
            "startup", "venture", "vc", "saas", "product management", "growth hacking",
            "funding", "founder", "ipo", "bourse", "wall street",
            "levée de fonds", "valorisation",
            "oracle", "spacex", "microsoft", "google", "meta", "amazon", "apple",
        ],
        "Carrière & développement personnel": [
            "career", "carrière", "skill", "compétence", "success",
            "luck", "timing", "attribution", "accidental",
            "freelance", "job", "hiring", "leadership", "management",
        ],
        "Créativité & arts": [
            "creativity", "creative", "créativité", "créatif",
            "art", "artist", "illustration", "drawing", "painting",
            "writing", "writer", "écriture", "auteur",
            "comics", "bd", "bande dessinée", "manga",
            "music", "musique", "photography", "photographie",
            "spring cleaning", "zoe sees", "frustration",
        ],
        "Design & UX": [
            "design", "ux", "ui", "figma", "typography", "interface",
            "visual", "branding", "photoshop", "procreate",
        ],
        "Actualités & société": [
            "blast, le souffle de l'info",
            "lemonde", "le monde", "blast", "blast-info", "actualité", "société", "good news",
            "réconfort", "fil good", "journal", "presse", "media", "news",
            "politique", "économie", "monde", "vélo", "cycliste", "cyclisme",
        ],
    }

    for nl in newsletters:
        # Build the haystack from all available text fields
        haystack = (nl.sender + " " + nl.subject + " " + nl.summary).lower()

        # Score each theme by counting keyword matches
        scores: dict[str, int] = {
            theme: sum(1 for kw in keywords if kw in haystack)
            for theme, keywords in THEME_RULES.items()
        }

        best_theme = max(scores, key=lambda t: scores[t])

        # Assign "Autres" if no keyword matched at all
        nl.theme = best_theme if scores[best_theme] > 0 else "Autres"
